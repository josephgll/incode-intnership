  ...
  switch (action.type) {
    ...
    case "ADDTOWORKOUT":
      state = {
        ...
        [action.payload[0]]: {
          ...
          exMeasureQ: [
            ...state[action.payload[0]].exMeasureQ,
            action.payload[4]
          ]
        }
      };
    ...
    case "WORKOUTMOVEUP":
      ...
      tempEx2.splice(
        action.payload[1] - 1,
        0,
        tempEx2.splice(action.payload[1], 1)[0]
      );
      tempExMeasure2.splice(
        action.payload[1] - 1,
        0,
        tempExMeasure2.splice(action.payload[1], 1)[0]
      );
      tempExRepeats2.splice(
        action.payload[1] - 1,
        0,
        tempExRepeats2.splice(action.payload[1], 1)[0]
      );
      tempExMeasureQ2.splice(
        action.payload[1] - 1,
        0,
        tempExMeasureQ2.splice(action.payload[1], 1)[0]
      );
      state = {
        ...state,
        [action.payload[0]]: {
    ...
    case "WORKOUTMOVEDOWN":
      ...
      tempEx3.splice(
        action.payload[1] + 1,
        0,
        tempEx3.splice(action.payload[1], 1)[0]
      );
      tempExMeasure3.splice(
        action.payload[1] + 1,
        0,
        tempExMeasure3.splice(action.payload[1], 1)[0]
      );
      tempExRepeats3.splice(
        action.payload[1] + 1,
        0,
        tempExRepeats3.splice(action.payload[1], 1)[0]
      );
      tempExMeasureQ3.splice(
        action.payload[1] + 1,
        0,
        tempExMeasureQ3.splice(action.payload[1], 1)[0]
      );
      state = {
        ...state,
        [action.payload[0]]: {
