...
class SignUp extends React.Component
{
  ...
  handleSubmit(e){
    if (this.state.password !== this.state.password2){
      ...
      e.preventDefault();
    } else if (this.state.password === this.state.password2 && this.state.email){
  ...
  <div style={{position: "absolute", background: "violet", width: 650, height: 90, marginTop: 180, marginLeft: 120}}>
    <h2 style={{margin: 10, color: "white" ,fontWeight: "normal" }}>Register With Fit Trainer App</h2>
  ...
  <div style={{background: "white", width: 700, height: 400, marginTop: 200, marginLeft: 100, borderRadius: "2%"}}>
    <form onSubmit={this.handleSubmit.bind(this)}>
      <div style={{display: "flex", width: "30%", flexDirection: "column"}}>
        ...
        <Input style={{display: "block", width: 320, marginLeft: 25, marginTop: 30}} placeholder="Repeat password" type="password" onChange={this.handlePasswordValue2.bind(this)} value={this.state.password2} required />
      </div>
      ...
      <Link to="/signin" style={{textDecoration: "none"}}><p style={{marginLeft: 25, color: "violet"}}>already have an account? sign in</p></Link>
    </form>
  </div>
